...
const FetchedProducts = () => {
	...
	async function getProducts() {
		...
		try {
			const response = await apiCall.getRequest(
				apiUrl.getLimitedProducts({ limit: 10 })
			);
			setLoading(false);
	...
	async function handleLoadMore() {
		try {
			const response = await apiCall.getRequest(
				apiUrl.getLimitedProducts({ limit: 10, skip: 10 })
			);
			setButtonIsHidden(true);
			setProductsData((prevData) => [...prevData, ...response.data.products]);
			return response.data.products;
		} catch (error: any) {
			console.error(error);
			setError(error?.message);
	...
	return (
		<div className="py-[24px]">
			<div className="grid place-content-center sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-[30px]">
				{isLoading ? (
			...
			<div className="flex items-center justify-center w-auto mt-10">
				<p
					...
					className={`${
						buttonIsHidden ? "hidden" : "block"
					} cursor-pointer text-[#23A6F0] text-[14px] font-bold rounded-[5px] py-[15px] px-[40px] border border-[#23A6F0]`}
				>
					{`${loading ? <Spinner /> : "LOAD MORE PRODUCTS"}`}
				</p>
			</div>
		</div>
	);
